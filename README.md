...
| Docker DNS 超时 | dockerd 直查已死路由器 (192.168.1.1:53 i/o timeout) | `daemon.json` 设 `dns`=公共 DNS (119.29.29.29/223.5.5.5/8.8.8.8)，与 CoreDNS 解耦 |
...
| Supervisor 永久 'No connectivity' | 把路由器网关 IP 写进 CoreDNS `.:53` forward → 网关反射查询触发 `plugin/loop` FATAL → CoreDNS 崩溃 | `.:53` 固定 `dns://127.0.0.11` + `daemon.json` dns=公共 DNS；绝不把网关写进 forward |
...
| **Host 系统 DNS** | host 指向 hassio_dns (`172.30.32.3`)，实际解析由 CoreDNS `.:53`→`127.0.0.11`→公共 DNS 完成（绝不直连路由器） | 公共 DNS 是全局固定 IP，与家庭网段/网关无关，天然适配任意路由器 |
| CoreDNS 上游 | `.:53`→`dns://127.0.0.11`（Docker 内嵌，走 daemon.json 公共 DNS）；`.:5553`→公共 DNS 替换被墙的 CloudFlare DoT | 公共 DNS 是全局固定 IP，与家庭网段/网关无关，彻底规避 loop 与"不同网段"问题 |
...
8123 端口长期停留在 landingpage、HA Core 永远起不来的**根因**，不是 Docker 镜像下载慢，而是 **supervisor 永久标记 `No Supervisor connectivity`**。而该标记背后的真正杀手，是 **CoreDNS `plugin/loop` 死循环导致 CoreDNS 崩溃**。

### 根因：把路由器网关写进 CoreDNS `.:53` forward → loop FATAL

Build #49 实测日志铁证：
1. 设备连路由器后，host 拿到路由器 DNS `192.168.1.1`。**该路由器 DNS 在本网会把查询反射回 `172.30.32.3`（CoreDNS 自身）**。
2. 早期修复曾把网关 IP 直接写进 CoreDNS `.:53` 的 `forward .` 列表。CoreDNS 启动自检时发现 `Loop (172.30.32.3:40880 -> :53) detected` → **FATAL 退出**。
...
> 关键认知：**路由器网关 IP 绝不能直连进 CoreDNS 的 `forward`**。无论网关是否可用，直连它都会触发 loop（可用时反射、不可用时 i/o timeout 只是其中一种表象）。

### 修复方案：对齐 stock HA OS 的 loop-free 架构（Build #51）

- **`.:53` forward 固定为 `dns://127.0.0.11`**（Docker 内嵌解析器）。`127.0.0.11` 自身不会反射，它走 daemon 的 DNS 配置，绝不 loop。
- **`daemon.json` 新增 `dns: [119.29.29.29, 223.5.5.5, 8.8.8.8]`**：让 dockerd 与 `127.0.0.11` 都走**公共 DNS**（同时修掉 dockerd 直查已死路由器导致的 i/o timeout）。
- **`.:5553`（supervisor 自用）只把被墙的 CloudFlare DoT (`tls://1.1.1.1`) 换成公共 DNS**。
- **公共 DNS 是全局固定 IP，与家庭网段 / 网关 IP 无关** → 天然适配任意路由器（192.168.x / 10.x / 172.x），彻底规避"不同家庭网段网关不同"的问题，也彻底消除 loop。

解析链路（无 loop）：
```
容器/supervisor ──> 172.30.32.3:53 (CoreDNS .:53) ──> 127.0.0.11 ──> 公共 DNS (119.29.29.29 等)
host 进程        ──> 172.30.32.3:53 (CoreDNS .:53) ──> 127.0.0.11 ──> 公共 DNS
```
（`127.0.0.11` 用的是 daemon.json 的 `dns`，不是 host 的 resolv.conf，因此不会绕回 CoreDNS 形成环。）

| 脚本 | 触发时机 | 作用 |
|------|----------|------|
| `etc/NetworkManager/dispatcher.d/99-hassos-dns-cn` | 网卡 up（最早，T≈10s） | 立即把 host DNS 指向 hassio_dns，并后台拉起主脚本 |
| `usr/libexec/hassos-dns-cn-init` | `hassos-dns-cn-init.service`（`After=hassos-supervisor`） | 主力：配 host DNS → `ha dns options` 公共 DNS → 修 Corefile（仅替换 `.:5553` DoT + 还原 `.:53` 为 127.0.0.11）→ healthcheck |
| `usr/libexec/hassos-dns-cn-init-supervisor-recheck` | `*.timer` 每 5 分钟保底 | 联网后重配（含 `ha dns options` 持久化）+ healthcheck，成功后自禁 timer |

> 三重保险确保无论 supervisor 何时就绪，CoreDNS 都处于 loop-free 的公共 DNS 状态，连通性检查能过关，HA Core 镜像正常拉起。
...
A: 最常见根因是 supervisor 永久标记 `No Supervisor connectivity`，其背后真正杀手是 **CoreDNS `plugin/loop` FATAL 崩溃**：早期版本曾把路由器网关 IP 写进 CoreDNS `.:53` 的 `forward .`，网关把查询反射回 CoreDNS 自身触发死循环，CoreDNS 启动自检 `Loop (... -> :53) detected` → FATAL 退出 → DNS 全死 → supervisor 永远无网络 → **永不拉取** `ghcr.io` 的 HA Core 镜像（不是下载慢，是根本不下载）。Build #51 已重构为 loop-free 架构（`.:53` 固定 `dns://127.0.0.11`、`daemon.json` dns=公共 DNS `119.29.29.29/223.5.5.5/8.8.8.8`、绝不把网关写进 forward）。若烧录新版本后仍卡住，查看串口日志是否出现 `plugin/loop` / `No Supervisor connectivity`，可在设备上手动执行：
```bash
# 1) 设置公共 DNS（持久化，reload 后仍在）
ha dns options --fallback=false --servers dns://119.29.29.29 --servers dns://223.5.5.5 --servers dns://8.8.8.8
# 2) 把被墙的 CloudFlare DoT 换成公共 DNS（不重启容器，避免 supervisor 重写）
docker exec hassio_dns sed -i 's|tls://1.1.1.1|dns://119.29.29.29|g' /etc/corefile
docker exec hassio_dns pkill -HUP coredns
# 3) 重拉 HA Core
ha core restart
```

**Q: DNS 解析失败 / 域名无法解析？**
A: 系统采用 loop-free 架构，与 stock HA OS 一致：host 与 supervisor 都通过 hassio_dns（`172.30.32.3`）解析，CoreDNS `.:53` 固定转发到 `127.0.0.11`（Docker 内嵌解析器，走 `daemon.json` 的公共 DNS），`127.0.0.11` 不会把查询反射回 CoreDNS，因此不会 loop；公共 DNS 是全局固定 IP，与家庭网段/网关无关，天然适配任意路由器。若仍解析失败，请检查：① `docker exec hassio_dns cat /etc/corefile` 的 `.:53` forward 是否为 `dns://127.0.0.11`（而非某个网关 IP）；② `docker logs hassio_dns` 是否仍有 `plugin/loop` FATAL；③ supervisor 是否仍报 `No Supervisor connectivity`（见上一条）。
